...
function Uploads ({ uploads, loading, onSelect, onNext, onPrev, onRefresh }: UploadsProps): JSX.Element {
    ...
    : (
      <div className='max-w-4xl'>
        ...
        <nav className='flex flex-row justify-between my-4'>
          <button onClick={e => { e.preventDefault(); onPrev && onPrev() }} className='inline-block bg-zinc-950 hover:outline text-white font-bold text-sm pl-3 pr-6 py-2 rounded-full whitespace-nowrap' disabled={!onPrev}>
            <ChevronLeftIcon className='h-5 w-5 inline-block mr-1 align-middle'/> Previous
          </button>
          <button onClick={e => { e.preventDefault(); onRefresh() }} className='inline-block bg-zinc-950 hover:outline text-white font-bold text-sm px-6 py-2 rounded-full whitespace-nowrap'>
            <ArrowPathIcon className={`h-5 w-5  ${loading ? 'animate-spin' : ''} inline-block mr-1 align-middle`}/> Reload
          </button>
          <button onClick={e => { e.preventDefault(); onNext && onNext() }} className='inline-block bg-zinc-950 hover:outline text-white font-bold text-sm pl-6 pr-3 py-2 rounded-full whitespace-nowrap' disabled={!onNext}>
